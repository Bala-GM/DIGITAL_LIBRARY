    ...
    if os.path.exists(path):
        ...
        if files:
            for file in files:
                ...
                file_listbox.insert("end", "")  # Add blank row for spacing
        else:
...
def open_file():
    try:
        ...
        if selected.strip() == "":
            showinfo("Info", "Please select a valid file.")
            return
...
header_label = Label(header_frame, text="Digital Library Manager", font=("Arial", 20, "bold"), bg="black", fg="green")
header_label.pack()
...
file_listbox = Listbox(
    list_frame, yscrollcommand=scrollbar.set, font=("Arial", 14), height=15, bg="black", fg="green", selectbackground="green"
)
...
list_btn = Label(btn_frame, text="List Files", font=("Arial", 14, "bold"), bg="green", fg="black", width=15, cursor="hand2")
list_btn.pack(side="left", padx=10)
list_btn.bind("<Button-1>", lambda e: list_files())

open_btn = Label(btn_frame, text="Open File", font=("Arial", 14, "bold"), bg="green", fg="black", width=15, cursor="hand2")
...
root.mainloop()
#8Q;P?y=?ELG7V#j
#pyinstaller -F -i digital-book.ico --noconsole DIGITAL_LIBRARY.py